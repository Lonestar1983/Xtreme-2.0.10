 ...
 ******************************************************/
	if ( (!$userdata['user_ftr']) && ($userdata['user_id'] != ANONYMOUS) )
	{
		...
		$directed = get_query_var( 'directed', 'get', 'string' );
		if ( $directed ) {
...
$sql = "SELECT mode FROM " . LOGS_TABLE . " WHERE last_post_id = '" . $postrow[ $i ]['post_id'] . "' ORDER BY log_id DESC LIMIT 1";
if ( ! $result = $db->sql_query( $sql ) ) {
	message_die(
		GENERAL_ERROR,
		'Could not get moved type',
		'',
		__LINE__,
		__FILE__,
		$sql
...
$row        = $db->sql_fetchrow($result);
$moved_type = isset( $row['mode'] ) ? $row['mode'] : '';
$select     = '';
