...
function query_ranks() {
	...
	if ( ! ( $result = dbquery( $sql ) ) ) {
		message_die(
			GENERAL_ERROR,
			"Could not obtain banned users information.",
			'',
			__LINE__,
			__FILE__,
			$sql
	...
	$ranks_sql                = array();
	$ranks_sql['bannedrow'][] = dbrowset( $result );
	dbfree( $result );
...
function generate_ranks( $user_row, $ranks_sql ) {
	...
	for( $j = 0; $j < count( $ranks_sql['ranksrow'] ); $j++ )
	{
		...
		$rank_img_tmp = ( $ranks_sql['ranksrow'][ $j ]['rank_image'] ) ? '<img src="modules/Forums/' . $ranks_sql['ranksrow'][ $j ]['rank_image'] . '" alt="">' : '';
		if ( $is_guest == true ) {
