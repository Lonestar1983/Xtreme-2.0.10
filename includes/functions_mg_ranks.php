...
function generate_ranks($user_row, $ranks_sql)
{
	...
	for($j = 0; $j < count($ranks_sql['ranksrow']); $j++)
	{
		...
		$rank_img_tmp = ( $ranks_sql['ranksrow'][$j]['rank_image'] ) ? '<img class="forum-ranks" src="modules/Forums/' . $ranks_sql['ranksrow'][$j]['rank_image'] . '" alt="' . $rank_tmp . '" title="' . $rank_tmp . '" border="0" />' : '';
		if ($is_guest == true)
		{
