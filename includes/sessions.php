...
function select_session_url($session_page, $url_qs, $url_ps, $specific, $level, $id, $forum_data, $topic_data, $user_data, $cat_data)
{
	...
	$query_string     = strip_session_sid();
		...
		if (isset($_GET[POST_USERS_URL]))
			$specific = intval($_GET[POST_USERS_URL]);

		if (isset($_GET[POST_FORUM_URL]))
			$specific = intval($_GET[POST_FORUM_URL]);

		if (isset($_GET[POST_GROUPS_URL]))
			$specific = intval($_GET[POST_GROUPS_URL]);

		if (isset($_GET[POST_CAT_URL]))
			$specific = intval($_GET[POST_CAT_URL]);

		if (isset($_GET[POST_TOPIC_URL]))
			$specific = intval($_GET[POST_TOPIC_URL]);

		if (isset($_GET[POST_POST_URL]))
			$specific = intval($_GET[POST_POST_URL]);
...
function session_begin($user_id, $user_ip, $page_id, $auto_create = 0, $enable_autologin = 0, $admin = 0) {
	...
	/**
	 * ? Set the cookie data for unread posts.
	 */
	setcookie(
		$cookiename . '_data',
		...
		array(
			...
			'path'     => trailingslashit( $cookiepath ),
			'domain'   => untrailingslashit( $cookiedomain ),
...
function session_pagestart( $user_ip, $thispage_id, $trash = 0 ) {
	...
	session_start();
	global $db, $lang, $board_config, $session_id, $SID, $pc_dateTime, $cookie;
	...
	if ( isset( $_GET['pc_tzo'] ) ) {
		$_SESSION['pc_timezoneOffset']    = (int) $_GET['pc_tzo'];
		$pc_dateTime['pc_timezoneOffset'] = $_SESSION['pc_timezoneOffset'];
		$server_time                      = gmdate( 'G' ) * 3600 + gmdate( 'i' ) * 60 + gmdate( 's' );

		if ( (int) $_GET['pc_d'] == gmdate('Ymd') ) {
			$_SESSION['pc_timeOffset'] =  (int) $_GET['pc_t'] - $server_time;
		} else {
			if ( (int) $_GET['pc_d'] > gmdate( 'Ymd' ) ) {
				$_SESSION['pc_timeOffset'] = ( 86400 - $server_time ) +  (int) $_GET['pc_t'];
			} else {
				$_SESSION['pc_timeOffset'] = - ( ( 86400 -  (int) $_GET['pc_t'] ) + $server_time );
			}
		}
	...
	} elseif ( isset( $_SESSION['pc_timezoneOffset'] ) ) {
		$pc_dateTime['pc_timezoneOffset'] = $_SESSION['pc_timezoneOffset'];
		$pc_dateTime['pc_timeOffset']     = $_SESSION['pc_timeOffset'];
	}
		...
		if ( isset( $userdata['user_id'] ) ) {
			...
			if ( $ip_check_s == $ip_check_u ) {
				...
				if ( $current_time - $userdata['session_time'] > 60 || $pc_dateTime_update == true ) {
					...
					$update_admin = ( ! defined( 'IN_ADMIN' )
						&& $current_time - $userdata['session_time'] > ( $board_config['session_length'] + 60 ) ) ? ', session_admin = 0' : '';
...
function session_end( $session_id, $user_id ) {
	global $db, $lang, $board_config, $userdata, $SID;
