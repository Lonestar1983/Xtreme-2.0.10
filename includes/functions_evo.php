...
function get_user_field( $field_name, $user, $is_name = false ) {
	...
	if ( ! isset( $actual_user[ $user ] ) ) {
		$actual_user[ $user ] = dburow( "SELECT * FROM " . USERS_TABLE . " WHERE $where" );
		// We also put the groups data in the array.
		$result = dbquery( 'SELECT g.group_id, g.group_name, g.group_single_user FROM (' . GROUPS_TABLE . ' AS g INNER JOIN ' . USER_GROUP_TABLE . ' AS ug ON ( ug.group_id = g.group_id AND ug.user_id = "' . $actual_user[ $user ]['user_id'] . '" AND ug.user_pending = 0 ) )' );
		while ( list( $g_id, $g_name, $single ) = dbrow( $result ) ) {
	...
	if ( is_array( $field_name ) ) {
		...
		foreach( $field_name as $field ) {
			$data[ $field ] = $actual_user[ $user ][ $field ];
		}
...
function get_admin_field( $field_name, $admin ) {
	...
	if ( ! isset( $fields[ $admin ] ) || !is_array( $fields[ $admin ] ) ) {
		$fields[ $admin ] = dburow( "SELECT * FROM " . _AUTHOR_TABLE . " WHERE aid = '" .  str_replace( "\'", "''", $admin ) . "'" );
	}
...
function is_mod_admin( $module_name='super' ) {
	...
	if ( $module_name != 'super' ) {
		list( $admins ) = dburow( "SELECT admins FROM " . _MODULES_TABLE . " WHERE title = '$module_name'" );
...
function get_mod_admins( $module_name = 'super', $all = '' ) {
	...
	if ( $module_name == 'super' || $all != '' ) {
		$result = dbquery( "SELECT aid, email FROM " . _AUTHOR_TABLE . " WHERE radminsuper = '1'" );
		$num = 0;
		while ( list( $admin, $email ) = dbrow( $result ) ) {
	...
	if ( $module_name != 'super' ) {
		list( $admin ) = dburow( "SELECT admins FROM " . _MODULES_TABLE . " WHERE title = '" . $module_name . "'" );
		$adminarray = explode( ",", $admin );
		$num = ( $all != '' ) ? $num : 0;
		for ( $i = 0, $maxi = count( $adminarray ); $i < $maxi; ++$i ) {
			$row = dburow( "SELECT aid, email FROM " . _AUTHOR_TABLE . " WHERE aid = '" . $adminarray[ $i ] . "'" );
